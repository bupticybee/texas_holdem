...
class TreeBuilder:
    ...
    def __gen_km_json(self,root,depth,limit):
        if limit is not None and depth > limit:
            return 
        children = []
        text = root.to_string()
        ...
        if root.children is not None:
            for one_child in root.children.values():
                child_json = self.__gen_km_json(one_child,depth + 1,limit)
                if child_json:
...
class TwoCardTexasTreeBuilder(TreeBuilder):
    def build_tree(self):
        players = list(range(self.rule.players))
        player = 0
        root = ActionNode(None,committed = self.rule.get_beginning_chip(),players = players,player = player,last_action='begin',bet_history = [])
        self.root = root
        return self.__build(root)
        
    def __build(self,root):
        if type(root) is ActionNode:
            self.__build_action(root)
    ...
    def __build_action(self,root):
        ...
        # the possible actions after certain action
        possible_actions = self.rule.legal_actions_after[root.last_action]
        ...
        checksum = sum([1 if i == 'check' else 0 for i in root.bet_history ])
        if checksum >= check_limit:
            return 
        
        for one_action in possible_actions:
            if one_action == 'check':
                committed = deepcopy(root.committed)
                if checksum >= check_limit - 1:
                    # 双方均check的话,进入摊派阶段
                ...
                committed = deepcopy(root.committed)
                if root.last_action == 'bet':
                    committed[player] += self.rule.amounts['bet']
                elif root.last_action == 'raise':
                    committed[player] += (self.rule.amounts['raise'] - self.rule.amounts['bet'])
                nextnode = ShowdownNode(root,committed = committed,players = players,player = next_player,last_action=one_action,bet_history = root.bet_history + [one_action])
...
class FiveCardTexasTreeBuilder(TreeBuilder):
    ...
    def get_possible_betting_sizes(self,root,player,next_player):
        ...
        sb = self.rule.amounts["small_blind"]
        bb = self.rule.amounts["big_blind"]
